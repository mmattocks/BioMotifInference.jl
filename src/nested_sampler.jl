...
function nested_step!(e::IPM_Ensemble, permute_instruction::Permute_Instruct)
    ...
    while !model_selected
        candidate,step_report=run_permutation_routine(e, permute_instruction)
...
function nested_step!(e::IPM_Ensemble, model_chan::RemoteChannel, worker_persistence::BitVector)
    N = length(e.models) #number of sample models/particles on the posterior surface
    i = length(e.log_Li) #iterate number, index for last values
    j = i+1 #index for newly pushed values
...
function ns_converge!(e::IPM_Ensemble, param_set, permute_limit::Integer, evidence_fraction::AbstractFloat=.001; model_display=1, backup::Tuple{Bool,Integer}=(false,0), verbose::Bool=false)
    N = length(e.models)
...
function ns_converge!(e::IPM_Ensemble, param_sets, permute_limit::Integer, writer::Integer, worker_pool::Vector{Integer}, evidence_fraction::AbstractFloat=.001; model_display::Integer=1, backup::Tuple{Bool,Integer}=(false,0), verbose::Bool=false)
    N = length(e.models)
    log_frac=log(evidence_fraction)
